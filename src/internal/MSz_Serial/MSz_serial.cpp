...
void computeAdjacency_cpu(std::vector<int> &adjacency, int width, int height,
...
int find_direction_cpu(const std::vector<double> *data,
  ...
  for (int index = 0; index < data_size; index++) {
    ...
    int y_diff =
        (largetst_index / (width)) % height - (index / (width)) % height;
...
void initializeWithIndex_cpu(std::vector<int> &label,
                             const std::vector<int> *direction_ds,
                             const std::vector<int> *direction_as) {
...
void getlabel_cpu(std::vector<int> &label, const std::vector<int> *direction_as,
                  const std::vector<int> *direction_ds, int &un_sign_ds_cpu,
                  int &un_sign_as_cpu, int width, int height, int depth,
                  int maxNeighbors) {
  ...
  for (int i = 0; i < data_size; i++) {
    ...
    if (cur != -1 and (*direction_as)[cur] != -1) {
      ...
      next_vertex = from_direction_to_index(cur, direc, width, height, depth,
                                            maxNeighbors);
    ...
    if (cur != -1 and label[cur * 2] != -1) {
      ...
      next_vertex1 = from_direction_to_index(cur, direc, width, height, depth,
                                             maxNeighbors);
...
void mappath_cpu(std::vector<int> &label, const std::vector<int> *direction_as,
                 const std::vector<int> *direction_ds, int width, int height,
                 int depth, int maxNeighbors) {
  ...
  while (h_un_sign_as_cpu > 0 or h_un_sign_ds_cpu > 0) {
    ...
    getlabel_cpu(label, direction_as, direction_ds, h_un_sign_as_cpu,
                 h_un_sign_ds_cpu, width, height, depth, maxNeighbors);
  }

  return;
};

void get_false_criticle_points_cpu(int &count_f_max, int &count_f_min,
                                   const std::vector<int> *adjacency,
                                   const std::vector<double> *decp_data,
                                   const std::vector<int> *or_direction_as,
                                   const std::vector<int> *or_direction_ds,
                                   std::vector<int> &false_min,
                                   std::vector<int> &false_max,
  ...
  for (auto i = 0; i < data_size; i++) {
    ...
    if ((is_maxima && (*or_direction_as)[i] != -1) or
        (!is_maxima && (*or_direction_as)[i] == -1)) {

      false_max[count_f_max++] = i;

    }

    else if ((is_minima && (*or_direction_ds)[i] != -1) or
             (!is_minima && (*or_direction_ds)[i] == -1)) {
...
int extract_critical_points_cpu(
    ...
    unsigned int critical_point_types, int width, int height, int depth,
    int neighbor_number) {
  ...
  for (int i = 0; i < data_size; i++) {
    ...
    if (extract_saddle && !is_maxima && !is_minima) {
      // A simplified saddle point detection using component counting of lower
      // neighbors
      std::vector<int> lower_neighbors;
      for (int index = 0; index < maxNeighbors; index++) {
        int j = adjacency[i * maxNeighbors + index];
        if (j != -1 &&
            ((*data)[j] < (*data)[i] || ((*data)[j] == (*data)[i] && j < i))) {
            ...
            while (head < q.size()) {
              ...
              for (size_t m = 0; m < lower_neighbors.size(); m++) {
                if (!visited[m]) {
                  ...
                  // Check if next is adjacent to curr by looking in curr's
                  // adjacency list
  ...
  if (compute_segmentation) {
    ...
    find_direction_cpu(data, &adjacency, direction_as, direction_ds, width, height, depth, maxNeighbors);
    
    mappath_cpu(labels, &direction_as, &direction_ds, width, height, depth, maxNeighbors);
  }

  return MSZ_ERR_NO_ERROR;
}

void initialization_cpu(std::vector<double> &d_deltaBuffer, int data_size,
                        double bound) {
  for (int i = 0; i < data_size; i++) {
...
void applyDeltaBuffer_cpu(const std::vector<double> *d_deltaBuffer,
                          const std::vector<double> *input_data,
                          std::vector<double> &decp_data, int data_size,
                          double bound) {
...
int fix_process_cpu(std::vector<int> *or_direction_as,
                    std::vector<int> *or_direction_ds,
                    std::vector<int> *de_direction_as,
                    std::vector<int> *de_direction_ds,
                    const std::vector<double> *input_data,
                    std::vector<double> *decp_data, std::vector<int> *dec_label,
                    std::vector<int> *or_label, int width, int height,
                    int depth, double bound, int preserve_min, int preserve_max,
  ...
  find_direction_cpu(input_data, &adjacency, *or_direction_as, *or_direction_ds,
                     width, height, depth, maxNeighbors);
  find_direction_cpu(decp_data, &adjacency, *de_direction_as, *de_direction_ds,
                     width, height, depth, maxNeighbors);
  ...
  mappath_cpu(*or_label, or_direction_as, or_direction_ds, width, height, depth,
              maxNeighbors);

  int count_f_max = 0;
  int count_f_min = 0;

  get_false_criticle_points_cpu(count_f_max, count_f_min, &adjacency, decp_data,
                                or_direction_as, or_direction_ds, false_min,
                                false_max, maxNeighbors, data_size);
  ...
  while (count_f_max > 0 or count_f_min > 0) {
    ...
    for (auto i = 0; i < count_f_max; i++) {
      ...
      fix_maxi_critical(input_data, decp_data, or_direction_as, or_direction_ds,
                        de_direction_as, de_direction_ds, d_deltaBuffer, width,
                        height, depth, maxNeighbors, bound, critical_i, 0);
    }
    ...
    for (auto i = 0; i < count_f_min; i++) {
      ...
      fix_maxi_critical(input_data, decp_data, or_direction_as, or_direction_ds,
                        de_direction_as, de_direction_ds, d_deltaBuffer, width,
                        height, depth, maxNeighbors, bound, critical_i, 1);
    }

    applyDeltaBuffer_cpu(&d_deltaBuffer, input_data, *decp_data, data_size,
                         bound);

    find_direction_cpu(decp_data, &adjacency, *de_direction_as,
                       *de_direction_ds, width, height, depth, maxNeighbors);
    get_false_criticle_points_cpu(
        count_f_max, count_f_min, &adjacency, decp_data, or_direction_as,
        or_direction_ds, false_min, false_max, maxNeighbors, data_size);
    if (preserve_max == 0)
      count_f_max = 0;
    if (preserve_min == 0)
      count_f_min = 0;
  ...
  mappath_cpu(*dec_label, de_direction_as, de_direction_ds, width, height,
              depth, maxNeighbors);

  std::vector<int> wrong_index_as;
  std::vector<int> wrong_index_ds;
  get_wrong_index_path(or_label, dec_label, wrong_index_as, wrong_index_ds,
                       data_size);

  while (wrong_index_as.size() > 0 or wrong_index_ds.size() > 0 or
         count_f_max > 0 or count_f_min > 0) {
    ...
    for (int i = 0; i < wrong_index_as.size(); i++) {
      ...
      fixpath(input_data, decp_data, or_direction_as, or_direction_ds,
              de_direction_as, de_direction_ds, d_deltaBuffer, j, 0, id_array,
              bound, width, height, depth, maxNeighbors);
    };
    ...
    for (int i = 0; i < wrong_index_ds.size(); i++) {
      ...
      fixpath(input_data, decp_data, or_direction_as, or_direction_ds,
              de_direction_as, de_direction_ds, d_deltaBuffer, j, 1, id_array,
              bound, width, height, depth, maxNeighbors);
    };
    ...
    while (count_f_max > 0 or count_f_min > 0) {
      ...
      for (auto i = 0; i < count_f_max; i++) {
        ...
        fix_maxi_critical(input_data, decp_data, or_direction_as,
                          or_direction_ds, de_direction_as, de_direction_ds,
                          d_deltaBuffer, width, height, depth, maxNeighbors,
                          bound, critical_i, 0);
      }
...
int count_false_cases_cpu(
    std::vector<int> *or_direction_as, std::vector<int> *or_direction_ds,
    ...
    std::vector<int> *dec_label, std::vector<int> *or_label, int width,
    int height, int depth, int neighbor_number, int &wrong_min, int &wrong_max,
    int &wrong_labels) {
